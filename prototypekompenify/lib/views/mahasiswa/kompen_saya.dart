...
class KompenSayaScreen extends StatelessWidget {
              ...
              child: RefreshIndicator(
                ...
                child: list.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (ctx, i) => KompenCard(
                          pengajuan: list[i],
                          onTap: () => _showDetail(ctx, list[i]),
                        ),
                      ),
              ),
            ),
  ...
  void _showDetail(BuildContext context, PengajuanKompen p) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => KompenDetailScreen(pengajuan: p)),
    );
  }
}
...
class KompenDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final p = svc
        .getPengajuan(mahasiswaId: pengajuan.mahasiswaId.toString())
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    InfoRow(
                      ...
                      value: DateFormat(
                        'dd MMM yyyy HH:mm',
                      ).format(p.tanggalPengajuan),
                    ),
                    if (p.catatanDosen != null)
                      InfoRow(
                        icon: Icons.comment_outlined,
                        label: 'Catatan Dosen',
                        value: p.catatanDosen!,
                      ),
                    if (p.catatanKaprodi != null)
                      InfoRow(
                        icon: Icons.comment_outlined,
                        label: 'Catatan Kaprodi',
                        value: p.catatanKaprodi!,
                      ),
                  ],
                ),
              ),
  ...
  String _statusDesc(KompenStatus s) {
    switch (s) {
      case KompenStatus.menunggu:
        return 'Upload bukti pengerjaan untuk melanjutkan';
      case KompenStatus.proses:
        return 'Menunggu verifikasi dari dosen';
      case KompenStatus.revisi:
        return 'Dosen meminta revisi, upload ulang bukti';
      case KompenStatus.disetujuiDosen:
        return 'Menunggu persetujuan akhir dari Kaprodi';
      case KompenStatus.lunas:
        return 'Kompen selesai! Surat siap dicetak';
      case KompenStatus.ditolak:
  ...
  Future<void> _uploadBukti(BuildContext context, PengajuanKompen p) async {
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            '✅ Bukti berhasil diupload! Menunggu verifikasi dosen.',
          ),
  ...
  void _cancelKompen(BuildContext context, PengajuanKompen p) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text(
          'Pengajuan kompen ini akan dihapus. Tindakan ini tidak dapat dibatalkan.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
  ...
  void _cetakSurat(BuildContext context, PengajuanKompen p) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Row(
          children: [
            Icon(Icons.description_rounded, color: AppTheme.accent),
            SizedBox(width: 8),
            Text('Surat Kompen'),
          ],
        ),
        content: Column(
          ...
          children: [
            ...
            _suratRow(
              'TTD Dosen',
              p.tanggalTtdDosen != null
                  ? '✅ ${DateFormat('dd/MM/yyyy').format(p.tanggalTtdDosen!)}'
                  : '-',
            ),
            _suratRow(
              'TTD Kaprodi',
              p.tanggalTtdKaprodi != null
                  ? '✅ ${DateFormat('dd/MM/yyyy').format(p.tanggalTtdKaprodi!)}'
                  : '-',
            ),
        ...
        actions: [
          ...
          ElevatedButton.icon(
            ...
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('📄 Surat kompen sedang diproses...'),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _ttdStep(
            'Upload Bukti',
            pengajuan.buktiFotoPath != null,
            'Bukti pengerjaan kompen',
          ),
          _ttdLine(pengajuan.buktiFotoPath != null),
          _ttdStep(
            'Verifikasi Dosen',
            pengajuan.ttdDosenBase64 != null,
            pengajuan.dosenNama,
          ...
          _ttdStep(
            'Persetujuan Kaprodi',
            pengajuan.ttdKaprodiBase64 != null,
            'Persetujuan akhir',
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(
                date != null ? DateFormat('dd MMM yyyy').format(date) : sub,
  ...
  Widget _ttdLine(bool done) {
    return Padding(
      ...
      child: Container(
        width: 2,
        height: 16,
        color: done ? AppTheme.accentGreen.withOpacity(0.5) : AppTheme.divider,
      ),
    );
  }
}
