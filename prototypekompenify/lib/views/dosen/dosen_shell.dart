...
class DosenVerifikasi extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final user = svc.currentUser!;
    // Konversi user.id ke String
    final list = svc.getPengajuan(dosenId: user.id.toString()).where((p) => p.status == KompenStatus.proses || p.status == KompenStatus.menunggu).toList();
    final history = svc.getPengajuan(dosenId: user.id.toString()).where((p) => p.status != KompenStatus.proses && p.status != KompenStatus.menunggu).toList();
