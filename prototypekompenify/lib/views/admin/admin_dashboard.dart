...
// Menggunakan AuthController untuk backward compatibility state pengguna dan penugasan area admin
...
class AdminDashboard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // 1. Mengakses state data pengguna terpusat lewat AuthController kelompokmu
    final svc = context.watch<AuthController>();
    final user = svc
        .staticCurrentUser; // Menggunakan data model static user bawaan AuthController

    // Jika user kosong, return widget kosong agar aman dari crash snapshot kernel
    if (user == null) return const SizedBox();

    final allUsers = svc.getUsers();
    final mahasiswas = allUsers
        .where((u) => u.role == UserRole.mahasiswa)
        .length;
          ...
          child: Column(
            ...
            children: [
              ...
              if (allUsers.length > 5) ...[
                ...
                Center(
                  child: Text(
                    'dan ${allUsers.length - 5} pengguna lainnya',
                    style: const TextStyle(
