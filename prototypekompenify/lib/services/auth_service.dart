...
class AuthService {
  ...
  Future<Map<String, dynamic>?> attemptLogin(
    ...
    try {
      final response = await http.post(
        ...
        headers: {
          'Accept':
              'application/json', // Memastikan Laravel selalu merespon dengan format JSON
  ...
  /// Fungsi baru untuk mengambil data profil teranyar dari MySQL Laravel
  Future<Map<String, dynamic>?> getLatestProfile(
    String token,
    String baseUrl,
  ) async {
    try {
      final response = await http.get(
        Uri.parse(
          '$baseUrl/profile',
        ), // Sesuaikan rute API profile di Laravelmu
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
          'Authorization':
              'Bearer $token', // Kirim token Sanctum sebagai jaminan akses
        },
      );
      ...
      print("Error di AuthService saat mengambil profil: $e");
      return null;
    }
  }
}
